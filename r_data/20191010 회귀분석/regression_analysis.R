...
json_df <- fromJSON(paste0(com_code[9],".json"))
...
x1 <- info_df$volume[length(info_df$volume)]+
      (info_df$volume[length(info_df$volume)]-info_df$volume[1])
x2 <- 120

result <- c(round(beta0+beta1*x1+beta2*x2,digits=-1)
            ,round(reg_result$adj.r.squared*100,digits=1))
result
