...
#감정분석 퍼센트 넣을 데이터 프레임 생성 및 이름 지정
emot_data <- data.frame(matrix(nrow=2, ncol=12)
                        ,row.names=c("positive","negative"))
names(emot_data) <- company
...
repeat{
  ...
  #5.위 3개 언론사에서 받은 기사들에 대한 감정분석 및 결과값 생성
  emot_anal_txt <- unlist(lapply(newstext,getMorph,"ALL"))
  refine_txt <- Filter(function(x){nchar(x)>=2 & nchar(x)<6},emot_anal_txt)
  
  pos_sum <- sum(!is.na(match(refine_txt,positive)))
  neg_sum <- sum(!is.na(match(refine_txt,negative)))
  sum <- pos_sum+neg_sum 
  pos_sum_per <- pos_sum/sum*100
  neg_sum_per <- neg_sum/sum*100
  
  emot_data[com_num] <- c(pos_sum_per,neg_sum_per)
