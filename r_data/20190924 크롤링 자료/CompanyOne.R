...
repeat{
  ...
  writeLines(unlist(savelist),file(paste0('파이낸셜_',company[com_num],'.txt')
                                   ,encoding='UTF-8'))
  savelist <- list()
  ...
  writeLines(unlist(savelist),file(paste0('한겨레_',company[com_num],'.txt')
                                   ,encoding='UTF-8'))
  savelist <- list()
  
  #3.중앙일보 뉴스에서 해당 종목 관련 기사 5개 크롤링
  site <- read_html(paste0('https://search.joins.com/JoongangNews?Keyword='
                           ,company[com_num],'&SortType=New&SearchCategoryType=JoongangNews&PeriodType=All&ScopeType=All&ImageType=All&JplusType=All&BlogType=All&ImageSearchType=Image&TotalCount=0&StartCount=0&IsChosung=False&IssueCategoryType=All&IsDuplicate=True&Page=1&PageSize=10&IsNeedTotalCount=True')
                    ,encoding="UTF-8")
  for(i in 1:5){
  ...
  writeLines(unlist(savelist),file(paste0('중앙일보_',company[com_num],'.txt')
                                   ,encoding='UTF-8'))
